...
const Dashboard2 = () => {
  ...
  useEffect(() => {
    const fetchPendingLoans = async () => {
      try {
        const response = await fetch('http://192.168.43.245:3001/loans/pending');
  ...
  const handleSave = async () => {
    try {
      const response = await fetch(`http://192.168.43.245:3001/loan/${editData.loanId}`, {
        method: 'PUT',
  ...
  const handleDelete = async (loanId) => {
    if (window.confirm('Are you sure you want to delete this loan?')) {
      try {
        await fetch(`http://192.168.43.245:3001/loan/${loanId}`, { method: 'DELETE' });
